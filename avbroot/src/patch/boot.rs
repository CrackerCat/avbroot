...
impl MagiskRootPatcher {
    ...
    const VER_SINGLE_BINARY: Range<u32> =
        27003..Self::VERS_SUPPORTED[Self::VERS_SUPPORTED.len() - 1].end;
...
impl BootImagePatch for MagiskRootPatcher {
    ...
    fn patch(&self, boot_image: &mut BootImage, cancel_signal: &AtomicBool) -> Result<()> {
        ...
        if Self::VER_SINGLE_BINARY.contains(&self.version) {
